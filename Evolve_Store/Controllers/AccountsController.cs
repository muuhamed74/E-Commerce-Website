    ...
    public class AccountsController : ControllerBase
    {
        ...
        public async Task<ActionResult> InitSignUpAsync([FromBody] RegisterDto model)
        {
            ...
            if (!result.IsAuthenticated)
                return BadRequest(new ApiResponce(400, result.Message));
        ...
        public async Task<IActionResult> ReSendCode([FromQuery] string email)
        {
            ...
            if (!result.IsSuccess)
                return BadRequest(new ApiResponce(400, "Email not found"));
        ...
        public async Task<ActionResult> AddRoleAsync([FromBody] AddRoleModel model)
        {
            ...
            if (!string.IsNullOrEmpty(result))
                return BadRequest(new ApiResponce (400, result));
        ...
        public async Task<ActionResult<UserDto>> RefreshToken()
        {
            ...
            if (!result.IsAuthenticated)
                return Unauthorized(new ApiResponce(401, result.Message));
        ...
        public async Task<ActionResult> RevokeToken([FromBody] RevokeTokenDto model)
        {
            ...
            if (string.IsNullOrEmpty(token))
                return BadRequest(new ApiResponce(400, "Token is required"));

            var result = await _iidentityservice.RevokeTokenAsync(token);

            if(!result)
                return BadRequest(new ApiResponce(400, "Token is invalid"));
        ...
        public async Task<IActionResult> Logout([FromQuery] string refreshToken)
        {
            ...
            return result ? Ok("Logged out successfully.") : BadRequest(new ApiResponce(401));
        }
        ...
        //    var MappedAdress = _mapper.Map<Adress ,AddressDto>(user.Adress);
        ...
        //[Authorize]
        //[HttpPut("Adress")]
        //public async Task<ActionResult<AddressDto>> UpdateUserAdress(AddressDto updatedaddress) 
        //{
        //    var user = await _userManager.FindUserWithAddressAsync(User);
        //    if (user == null) return Unauthorized(new ApiResponce(401));
        //    var address = _mapper.Map<AddressDto, Adress >(updatedaddress);
        ...
        ////Dublicate Emails At Registration
        //[HttpGet("EmailExists")]
